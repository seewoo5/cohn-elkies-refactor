...
Formalized as the structure `PeriodicSpherePacking`, which extends `SpherePacking` by the lattice
(a $`\mathbb{Z}`-submodule with the instances `DiscreteTopology` and `IsZLattice ℝ`) and the
invariance `lattice_action`; the periodic constant is `PeriodicSpherePackingConstant`, the orbit
count is `PeriodicSpherePacking.centerOrbitCardinality`, and its identification with the number
of centers in a fundamental domain is
`PeriodicSpherePacking.encard_centers_in_fundamental_region`.
:::
...
Formalized as `SchwartzMap.polarIntegerLattice`, with the covolume `ZLattice.covolume` from
Mathlib, the standard lattice `SchwartzMap.referenceIntegerLattice` and its self-duality
`SchwartzMap.PoissonSummation.Standard.dualSubmodule_referenceIntegerLattice`, and the coordinate
automorphism `SchwartzMap.latticeCoordinateEquiv` with the three properties
`SchwartzMap.map_referenceIntegerLattice`, `SchwartzMap.lattice_covolume_eq_coordinate_determinant`
and `SchwartzMap.map_referenceIntegerLattice_dual`.
:::
...
supremum over periodic packings of separation $`1`. Formalized as
`SpherePacking.packing_supremum_eq_unit_separation` and
`periodic_packing_supremum_eq_unit_separation`.
:::
...
with the covolume of {uses "def_polar_lattice"}[]. Formalized as
`PeriodicSpherePacking.upperPackingDensity_eq_div_volume_fundamentalDomain` and, with the
covolume, `PeriodicSpherePacking.density_eq_numReps_mul_volume_ball_div_covolume`; a periodic
packing without centers has density $`0` (
`PeriodicSpherePacking.packing_density_zero_of_empty_centers`).
:::
...
both series converging absolutely. Formalized as `SchwartzMap.latticePoissonSummationFormula`;
the case $`\Lambda = \mathbb{Z}^d` is
`SchwartzMap.PoissonSummation.Standard.poissonSummation_referenceIntegerLattice`.
:::
...
Formalized as `LinearProgrammingBound'`, in which the quotient is
formed from the nonnegative parts of $`f(0)` and $`\widehat f(0)` (both are positive, by
`test_function_positive_at_origin` and `fourier_transform_nonzero`).
:::
...
(`SpherePacking.exists_periodic_unit_packing_above_density_threshold`).
:::
...
with $`\Delta_d` from {uses "def_packing_density"}[]. No radiality is assumed. Formalized as
`LinearProgrammingBound`, with the same conventions as
{bpref "thm_lp_bound_periodic"}[].
:::
